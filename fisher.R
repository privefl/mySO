...
library(bigmemory)
M <- 10000
matrix <- matrix(sample(0:1, 100 * M, replace = TRUE), 100 , M)
bigmatrix <- as.big.matrix(matrix)
class <- sample(LETTERS[1:2], 100, replace = TRUE)
system.time(
  pval0 <- sapply(seq_len(ncol(bigmatrix)), function(i) {
    fisher.test(factor(bigmatrix[, i], levels = c(0, 1)), class)$p.value
  })
) # 7 sec
